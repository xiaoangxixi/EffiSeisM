...
def vis_phase_pickingv2(
        waveforms: np.ndarray,
        waveforms_labels: list,
        preds: np.ndarray,
        true_phase_idxs: list = None,
        true_phase_labels: list = None,
        pred_phase_labels: list = None,
        sampling_rate: int = None,
        save_name="",
        save_dir="./",
        formats=["png"],
    ...
    tmp_min = np.min(waveforms)
    tmp_max = np.max(waveforms)
    number_map = {0: "(I)", 1: "(II)", 2: "(III)", 3: "(IV)"}

    for idx, wave in enumerate(waveforms):
        plt.subplot(num_row, 1, idx + 1)
        plt.plot(x, wave, "-", color="k", linewidth=1.2, alpha=0.9, label=waveforms_labels[idx])

        if idx == 0 and true_phase_idxs:
            plt.vlines(x=[true_phase_idxs[0]], ymin=tmp_min * 1.1, ymax=tmp_max * 1.1, colors=["C1"],
                       linestyles="solid", label=true_phase_labels[0])
            plt.vlines(x=[true_phase_idxs[1]], ymin=tmp_min * 1.1, ymax=tmp_max * 1.1, colors=["C5"],
                       linestyles="solid", label=true_phase_labels[1])
    ...
    # Plot predictions in the last subplot (d)
    plt.subplot(num_row, 1, num_row)
    plt.text(0.05, 0.78, number_map[3], horizontalalignment='center',
             transform=plt.gca().transAxes, fontsize=8, fontweight="normal", bbox=None)

    # Line for D
    plt.plot(x, preds[0], f'-.C0', linewidth=1, alpha=0.9, label=pred_phase_labels[0])
    plt.ylabel('Probability')

    # Line for P
    plt.plot(x, preds[1], linestyle='--', color='orange', linewidth=1, alpha=0.9, label=pred_phase_labels[1])

    # Line for S
    plt.plot(x, preds[2], linestyle='--', color='red', linewidth=1, alpha=0.9, label=pred_phase_labels[2])
    plt.ylabel('Probability')
