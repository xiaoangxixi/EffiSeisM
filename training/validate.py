...
def save_differences(differences, model_name, filename='fig6.csv'):
    try:
        df = pd.read_csv(filename)
    except FileNotFoundError:
        df = pd.DataFrame()

    new_differences = pd.DataFrame(differences, columns=[f'{model_name}'])
    df = pd.concat([df, new_differences], axis=1)
...
def validate(
    ...
    model_labels, tgts_trans_for_loss, outs_trans_for_loss, outs_trans_for_res = Config.get_model_config_(
        args.model_name, "labels", "targets_transform_for_loss", "outputs_transform_for_loss",
    ...
    with torch.no_grad():
        for step, (x, loss_targets, metrics_targets, meta_data_jsons) in enumerate(val_loader):
            ...
            for task in tasks:
                ...
                if task in metrics_targets:
                    differences = (metrics_targets[task].to(device) - results[task].to(device)).cpu().numpy()
                    all_differences.extend(differences.flatten())  #记录误差
    ...
    # # 保存差异到 CSV
    save_differences(all_differences,model_name="MagNet", filename="fig6.csv")
    # if all_differences:
    #     plt.figure(figsize=(10, 6))
    #     plt.hist(all_differences, bins=50, alpha=0.7, color='blue')
    #     plt.xlabel('Residual')
    #     plt.ylabel('Number of samples')
    #     plt.xlim(-1.5, 1.5)  # 控制x轴范围
    #     plt.grid(False)  # 去掉网格
    #
    #     plt.show()
    ...
    if results_saver is not None:
        results_save_path = get_safe_path(
            os.path.join(logger.logdir(), f"test_results_{val_loader.dataset.name()}.csv"))
