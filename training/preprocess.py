...
import numpy as np
import json
from datasets._factory import build_dataset
        ...
        for name in task_names:
            if name in ["ppk", "spk"]:
                ...
                if len(detections)//2< expected_num:
                    # # Calculate the number of additional elements needed for padding
                    # additional_elements = expected_num * 2 - len(detections)
                    # # Extend detections list with [1, 0] pairs to reach the expected length
                    # detections.extend([1, 0] * additional_elements)
...
class SeismicDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[Any, Any, Any, Any]:
        ...
        #meta_data_json = json.dumps(meta_data.tolist())
